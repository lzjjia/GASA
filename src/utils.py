...
class TSNECallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, module, outputs):
        ...
        if trainer.is_global_zero and len(data_target):
            ...
            Y = torch.cat((Y_source, Y_target)).numpy()

            data = TSNE(n_components=2).fit_transform(torch.cat((data_source, data_target)).numpy())
            # assing to dataframe
            df = pd.DataFrame()
            df["feat_1"] = data[:, 0]
            df["feat_2"] = data[:, 1]
...
def compute_acc(y, y_hat, device):
    acc = y == y_hat
    if len(acc) > 0:
        acc = acc.sum().detach().true_divide(acc.size(0))   # true_divide means /
    else:
