...
class Block(nn.Module):
    ...
    def forward(self, x, return_attn=False):
        if return_attn:
            tmp_x, pre_drop_attn = self.attn(self.norm1(x), return_attn=return_attn)
            x = x + self.drop_path(tmp_x)
        else:
            x = x + self.drop_path(self.attn(self.norm1(x)))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        if return_attn:
            return x, pre_drop_attn
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)[:, 0]
        x = self.pre_logits(x)
        return x    # (B*frames, 768)

    def forward_features_att(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        for blk in self.blocks[:-1]:
            x = blk(x)
        x, spatial_attn = self.blocks[-1](x, return_attn=True)
    ...
    def forward_att(self, x):
        x, spatial_attn = self.forward_features_att(x)
        if self.aggregate:
            x, temporal_attn = self.aggregate.forward_att(x)
        x = self.head(x)
        return x, spatial_attn, temporal_attn

    def forward(self, x):
        # batch, _, _, frame, _, _ = x.size()
        # x=x.unsqueeze(0)
        x = rearrange(x, "b n_clips c f h w -> (b n_clips f) c h w")
        x = self.forward_features(x)   # (B*frames, 768)
        if self.aggregate:
            x = self.aggregate(x)      # (B, 768)
        return x
...
def STAM_224(model_params):
    ...
    model = VisionTransformer(
        img_size=img_size, num_classes=num_classes, aggregate=aggregate, **model_kwargs
    )
    return model
