...
def video_collate(batch):
    ...
    targets = torch.tensor(
        [b[2] for b in batch]
    )  # change to 1 because the original kinetics returns frames, audio,target
    ...
    if is_np:
        ...
        for i in range(batch_size):
            for t in range(T):
                tensor_uint8_CHW[i * T + t] = torch.from_numpy(batch[i][0][t]).permute(2, 0, 1)
...
def create_dataloader(args):
    ...
    if args.input_size == 448:  # squish
        val_tfms = transforms.Compose([transforms.Resize((args.input_size, args.input_size))])
    else:  # crop
            ...
            if last_batch or batch_idx % 100 == 0:
                ...
                print(
                    ...
                    "Prec@5: {top5.val:>7.2f} ({top5.avg:>7.2f})".format(
                        log_name, batch_idx, last_idx, top1=prec1_m, top5=prec5_m
                    )
                )
