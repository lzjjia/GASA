...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None,
        ...
        k = k.contiguous().view(src_len, self.num_heads, self.head_dim).transpose(0, 1)
        v = v.contiguous().view(src_len, self.num_heads, self.head_dim).transpose(0, 1)
        ...
        if self.add_zero_attn:
            src_len += 1
            k = torch.cat([k, torch.zeros((self.num_heads, 1, self.head_dim), dtype=k.dtype, device=k.device)], 1)
            v = torch.cat([v, torch.zeros((self.num_heads, 1, self.head_dim), dtype=v.dtype, device=v.device)], 1)
        ...
        assert list(attn_output.size()) == [self.num_heads, tgt_len, self.head_dim], attn_output.size()
        attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, self.embed_dim)
