...
def info_nce(logits, pos_mask, logit_mask=None, temperature=0.2):
    ...
    div = torch.sum(torch.exp(logits) * logit_mask, dim=1, keepdim=True)  # fen mu
    ...
    mean_log_prob_pos /= pos_mask.sum(1)  # divide the number of positives
...
class InterDomainBasedNCE(nn.Module):
    ...
    def forward(self, xs, xt, ys, yt, selection=None):
        ...
        if selection is not None:  # choose label is reliable simple
            x = x[selection]
            y = y[selection]
        ...
        # view_matrix = v.reshape(1, -1).repeat(b, 1)
        # view_mask = view_matrix != view_matrix.t()  # different view is positive

        labels_matrix = y.reshape(1, -1).repeat(b, 1)
        pos_mask = labels_matrix == labels_matrix.t()  # same label is positive
        pos_mask.fill_diagonal_(False)  # self is not positive
        pos_mask[:bs, :bs] = False  # same source simple is not positive
        pos_mask[bs:, bs:] = False  # same target simple is not positive

        logit_mask = torch.ones_like(logits, dtype=bool, device=xs.device)
        logit_mask.fill_diagonal_(False)
        logit_mask[:bs, :bs] = False
        logit_mask[bs:, bs:] = False  # logit_mask is True when simple is different domain
        if (logit_mask == 0).all():
            loss = torch.tensor(0.0, device=device)
        else:
